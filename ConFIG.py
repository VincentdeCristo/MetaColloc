"""
Baseline: Physics-Informed Neural Network (PINN) with ConFIG
Paper: CONFIG: TOWARDS CONFLICT-FREE TRAINING OF PHYSICS INFORMED NEURAL NETWORKS (ICLR 2025)
...
Key implementations of ConFIG:
1. Computes gradients of PDE residual (g1) and boundary (g2) separately.
2. Normalizes gradients: U_i = g_i / ||g_i||
3. Solves for direction x such that U x = 1_m using pseudo-inverse: x = U^+ 1_m
4. Normalizes x to get the conflict-free direction g_u.
5. Scales g_u by the sum of projections: mag = sum(g_i^T g_u).
6. Final gradient g_config = mag * g_u is fed into Adam.
"""
...
# ================================================================
# Solve one PDE using ConFIG
# ================================================================
def solve_pinn_config(pde_name, width, seed):
    ...
    # ConFIG strictly uses Adam with CosineAnnealingLR (from 1e-3 to 1e-4) based on Appendix A.12
    ...
    for step in range(max_iter):
        ...
        # ---------------------------------------------------------
        # Step 3: ConFIG Operator (Eq. 2, Eq. 3 & Appendix A.3)
        # ---------------------------------------------------------
        # G represents the unnormalized gradient matrix of shape (m, k). Here m = 2.
        G = torch.stack([grad_r, grad_b], dim=0)

        # U = [U(g_1), U(g_2)]^T  (Normalized gradient matrix)
        norms = torch.norm(G, dim=1, keepdim=True) + eps
        U = G / norms

        # We need to solve: U x = 1_m. The least squares solution is x = U^+ 1_m.
        # Since k (parameters) >> m (losses), U has full row rank. 
        # U^+ = U^T (U U^T)^{-1}. This avoids massive SVD on large dimensions.
        U_Ut = torch.matmul(U, U.T)
        jitter = 1e-8 * torch.eye(2, dtype=G.dtype, device=G.device)  # Add jitter for stability
        inv_U_Ut = torch.linalg.inv(U_Ut + jitter)
        U_pinv = torch.matmul(U.T, inv_U_Ut) # Shape: (k, 2)

        ones = torch.ones(2, 1, dtype=G.dtype, device=G.device)
        x = torch.matmul(U_pinv, ones).squeeze() # Shape: (k,)

        # Unit vector g_u
        g_u = x / (torch.norm(x) + eps)

        # Calculate final scaled magnitude: sum_{i=1}^m g_i^T g_u
        mag = torch.sum(torch.matmul(G, g_u)) 

        # Final Conflict-Free Inverse Gradient
        g_config = mag * g_u

        # ---------------------------------------------------------
        # Step 4: Apply g_config to Adam and step
        # ---------------------------------------------------------
        optimizer.zero_grad()
        idx = 0
        for p in model.parameters():
            numel = p.numel()
            p.grad = g_config[idx : idx + numel].view(p.shape).clone()
            idx += numel
...
for pde in PDE_NAMES:
    for h in D_HIDDENS:
        for seed in SEEDS:
            ...
            mse = solve_pinn_config(pde, h, seed)
            solve_times[pde][h].append(time.time() - t_solve_start)
            rmse = np.sqrt(mse)
            results[pde][h].append(rmse)
            print(f"[PINN+ConFIG] {pde} H={h} seed={seed} RMSE={rmse:.2e}")
